...
Public Class RayRender
    ...
    Public Sub DoRayRender()
        ...
        'Create the texture as Usage.Dynamic since it will be modified on every frame
        'Allocating the texture in Pool.Default will give faster performance but will require locking
        Dim texture As New Texture(Sim.Render.Device, Sim.Config.Render.Width, Sim.Config.Render.Height, 1, Usage.Dynamic, Format.A8R8G8B8, Pool.Default)
        Dim sprite As New Sprite(Sim.Render.Device)
        Dim spriteRect As DataRectangle

        Try
            'Data loaded from Simulation - Constant
            Dim HalfRenderWidth As Double = Sim.Config.Render.Width / 2
            Dim HalfRenderHeight As Double = Sim.Config.Render.Height / 2
            ...
            Dim PixelData((Sim.Config.Render.Width * Sim.Config.Render.Height * 4) - 1) As Byte
            Dim PixelDataByteIndex As Integer
            Dim PixelDataStride As Integer = Sim.Config.Render.Width * 4 '4 bytes per pixel becauase the format is A8R8G8B8
            ...
            Do While Sim.Running
                ...
                PixelDataByteIndex = 0

                'For each pixel
                For YPixel = 0 To Sim.Config.Render.Height - 1
                    ...
                    For XPixel = 0 To Sim.Config.Render.Width - 1
                        ...
                        If Ray_ClosestObject = -1 Then 'If no object was found return the background
                            ...
                            PixelData(PixelDataByteIndex) = BackgroundColorB
                            PixelData(PixelDataByteIndex + 1) = BackgroundColorG
                            PixelData(PixelDataByteIndex + 2) = BackgroundColorR
                            PixelData(PixelDataByteIndex + 3) = BackgroundColorA
                            PixelDataByteIndex += 4
                        Else
                            ...
                            'Write the pixel to the pixel array
                            PixelData(PixelDataByteIndex) = ToByte(Min(Ray_colorB, MaxByteD)) 'Cap the components at 255
                            PixelData(PixelDataByteIndex + 1) = ToByte(Min(Ray_colorG, MaxByteD))
                            PixelData(PixelDataByteIndex + 2) = ToByte(Min(Ray_colorR, MaxByteD))
                            PixelData(PixelDataByteIndex + 3) = 255
                            PixelDataByteIndex += 4
                        End If
                    Next
                Next
                ...
                spriteRect = texture.LockRectangle(0, LockFlags.None)
                For YPixel = 0 To Sim.Config.Render.Height - 1
                    Marshal.Copy(PixelData, YPixel * PixelDataStride, spriteRect.DataPointer + (spriteRect.Pitch * YPixel), PixelDataStride)
                Next
                ...
                SyncLock LockRayMulti 'Make sure only one thread is using the device 
                    ...
                    sprite.Draw(texture, Color.White)
                    sprite.End()
    ...
    Public Shared Sub InitializeRayRender(sim As SimulationRuntime)
